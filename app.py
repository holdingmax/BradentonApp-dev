...
try:
    from eft_cta_cte import (
        EFT_DUPLICATE_ALERT,
        eft_already_loaded_in_workbook,
        extract_eft_data,
        update_excel_workbook,
    )
except ImportError as _eft_exc:
    EFT_DUPLICATE_ALERT = ""
    eft_already_loaded_in_workbook = None
    extract_eft_data = None
    update_excel_workbook = None
    _EFT_IMPORT_ERROR = _eft_exc
else:
    _EFT_IMPORT_ERROR = None
...
try:
    from chase_rules import (
        ...
        CHASE_ADMIN_FEE_KEYWORDS,
        CHASE_AGUA_KEYWORDS,
        CHASE_ALARMA_KEYWORDS,
        CHASE_ALG_DISTR_KEYWORDS,
        CHASE_ELISTAR_KEYWORDS,
        CHASE_ENERGIA_KEYWORDS,
        CHASE_GASTOS_BANCARIOS_KEYWORDS,
        CHASE_PROVEEDORES_KEYWORDS,
        CHASE_REBATE_KEYWORDS,
        CHASE_SALE_TAX_KEYWORDS,
        CHASE_SEGURO_KEYWORDS,
        CHASE_TELEFONO_KEYWORDS,
        add_dynamic_rule,
        delete_dynamic_rule,
    ...
    process_chase_categorization = None
...
class EFTExtractorApp:
    ...
    def process_and_update(self):
        if extract_eft_data is None:
            messagebox.showerror(
                "Error",
                f"Módulo de EFT no disponible: {_EFT_IMPORT_ERROR}",
            )
            self._set_status(
                f"Módulo de EFT no disponible: {_EFT_IMPORT_ERROR}", is_error=True
            )
            return
    ...
    def process_chase_data(self):
        if process_chase_categorization is None:
            messagebox.showerror(
                "Error",
                f"Módulo de Chase no disponible: {_CHASE_RULES_IMPORT_ERROR}",
            )
            self._set_chase_status(
                f"Módulo de Chase no disponible: {_CHASE_RULES_IMPORT_ERROR}",
                is_error=True,
